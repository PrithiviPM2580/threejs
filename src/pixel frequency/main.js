...
function init() {
  ...
  material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uDisplacement: new THREE.Uniform(CONFIG.strength),
...
function createRecorder(stream, mimeType) {
  const optionSets = [];
  if (mimeType)
    optionSets.push({ mimeType, videoBitsPerSecond: CONFIG.bitrate });
...
function setupGUI() {
  ...
  ef.add(CONFIG, "layers", 2, 50, 1).onChange(
...
function animate() {
  ...
  if (CONFIG.autoRotate && mesh) {
    ...
    mesh.rotation.y =
      Math.sin(t * CONFIG.rotateSpeed) * THREE.MathUtils.degToRad(6);
  }
  renderer.render(scene, camera);
}
