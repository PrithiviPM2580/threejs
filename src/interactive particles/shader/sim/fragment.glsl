...
void main() {
   ...
   float targetRadius= mix(info.x,1.8,0.5+0.45*sin(angle * 2.0 + uTime * 0.2));
   radius += (targetRadius - radius) * mix(0.01,0.1,circularForce);
   vec3 targetPos= vec3(cos(angle),sin(angle),0.0) * radius;
  
   pos.xy += (targetPos.xy - pos.xy) * 0.1;

   pos.xy += curl(pos.xyz * 4.0,uTime*0.1,0.1).xy * 0.006;
